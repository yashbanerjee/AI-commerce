...
-- One-time move from legacy Bedrock defaults; safe if re-run (no matching rows).
UPDATE tenants SET embedding_provider = 'openai', llm_provider = 'openai'
WHERE embedding_provider = 'bedrock' OR llm_provider = 'bedrock';
UPDATE tenants SET openai_key_ciphertext = NULL WHERE openai_key_ciphertext IS NOT NULL;
